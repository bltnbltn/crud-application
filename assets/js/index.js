...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const createTaskForm = document.getElementById('write_article');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const updateTaskForm = document.getElementById('update_article');
...
const updateTask = async (data) => {
  ...
  try {
    ...
    const response = await fetch(`http://localhost:7000/api/articles/${data.id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`,
